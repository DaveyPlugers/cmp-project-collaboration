...
parser.add_argument('--Mass_Ratio',
                    help='Mass_Ratio of two galaxy, galaxy2/galaxy1, default: 1',
                    nargs=1)
...
if args.Mass_Ratio is None:
    ...
    NMass = float(args.Mass_Ratio[0])


# Write a txt output profile
f = open(folder1 + '\\' + 'output.txt', 'w')
f.write('Initial condition: Particle_Number = ' + str(N*4+math.floor(N*4*NMass)) + ' Steps = ' + str(Steps))
...
def Make_Galaxy(Number_disk, Mass_disk, Number_Bulge, Mass_Bulge, Shift_x, Shift_y, Shift_z, Shift_vx, Shift_vy,
    ...
    for i in range(Number_disk):
        ...
        Position_Mass = []
        Velocity = []
        Position_Mass.append(Radius * np.cos(theta) + Shift_x)
        Position_Mass.append(Radius * np.sin(theta) + Shift_y)
    ...
    for i in range(Number_Bulge):
        ...
        Position_Mass = []
        Velocity = []
        Position_Mass.append(Radius * np.sin(phi) * np.cos(theta) + Shift_x)
        Position_Mass.append(Radius * np.sin(phi) * np.sin(theta) + Shift_y)
        Position_Mass.append(Radius * np.cos(phi) + Shift_z)
...
def CoM_Separation(N_Galaxy1, N_Galaxy2):
    """
    Calculates the distance of the center of mass of 2 particle sets
    :param N_Galaxy1: Amount of particles in galaxy 1
    :param N_Galaxy2: Amount of particles in galaxy 2
    :return: Distance between the center of masses
    """
    CoM_Galaxy1 = [0.0, 0.0, 0.0, 0.0]
    CoM_Galaxy2 = [0.0, 0.0, 0.0, 0.0]
    for i in range(N_Galaxy1):
        CoM_Galaxy1 = CoM_Updater(CoM_Galaxy1, Particles[i][0])
    for j in range(N_Galaxy2):
        CoM_Galaxy2 = CoM_Updater(CoM_Galaxy2, Particles[N_Galaxy1 + j][0])
...
N1d = 3*N
N1b = N
N2d = math.floor(3*N*NMass)
N2b = math.floor(N*NMass)
N_Galaxy1 = N1d + N1b
N_Galaxy2 = N2d + N2b
M1d = 1 * NFold
...
Galaxy1 = Make_Galaxy(N1d, M1d, N1b, M1b, -10.0, 0.0, 0.0, 0, 0, 0)
Galaxy2 = Make_Galaxy(N2d, M2d, N2b, M2b, 10.0, 0.0, 0.0, 0, 0, 0)
Particles = Galaxy1
Particles += Galaxy2
...
def init():
    ...
    ax.legend(["Galaxy 1", "Galaxy 2"], loc="upper right")


def update(q):
    ln1.set_data([Particle_Pos_History[int(q)][0:N1d + N1b, 0]], [Particle_Pos_History[int(q)][0:N1d + N1b, 1]])
    ln2.set_data([Particle_Pos_History[int(q)][N1d + N1b:N1d + N1b + N2d + N2b, 0]],
                 [Particle_Pos_History[int(q)][N1d + N1b: N1d + N1b + N2d + N2b, 1]])

ani = FuncAnimation(fig, update, frames=int(Steps), interval=20, init_func=init)
ani.save(folder + '\\' + 'animation.gif', writer='pillow')
plt.figure(2)
plt.scatter(Plot_Particle1[:, 0, 0], Plot_Particle1[:, 0, 1], marker='.', label='Galaxy 1')
plt.scatter(Plot_Particle2[:, 0, 0], Plot_Particle2[:, 0, 1], marker='.', label='Galaxy 2')
