...
for i in range(Number_disk):
    ...
    while Too_Close:
        ...
        for j in range(len(Particle)):
            ...
            if DistanceParticles(np.array(Particle[j][0][0:3]),np.array([r*np.cos(theta),r*np.sin(theta),0])) < Minimum_Distance:
                Too_Close = True
...
print(Particle)
PlotParticle = np.array(Particle)
fig1 = plt.figure()
plt.scatter(PlotParticle[:,0,0], PlotParticle[:,0,1], marker='.')
...
ax.scatter(PlotParticle[:,0,0], PlotParticle[:,0,1], PlotParticle[:,0,2])
    ...
    for i in range(len(Particle)):
        ...
        for k in range(3):
            if abs(Particle[i][0][k]) > Length:
                Particle[i]= [[random.uniform(-Max_R/2,Max_R/2),random.uniform(-Max_R/2,Max_R/2),random.uniform(-Max_R/2,Max_R/2),0.0000001],[0.0,0.0,0.0]]
        while Looping:
...
def VelocityUpdate(Timestep):
    ...
    for i in range(len(Particle)):
        ...
        for k in range(3):
            Particle[i][1][k] += Timestep*Force[k]*Unit_Converter_Dist**2*Unit_Converter_Mass/Unit_Converter_Velocity**2
